...
aggregateRouter.get('/:sensor/summary', async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!(VALID_AGG_FUNCTIONS as readonly string[]).includes(fn)) {
        res.status(400).json({
    ...
    const sql =
        `SELECT ${fn.toUpperCase()}("${col}") AS result` +
