...
const DEFAULT_THRESHOLD_PERCENT = config.redline?.thresholdPercent ?? 80;
const WARNING_PERCENT = config.redline?.warningPercent ?? 10;
const DANGER_PERCENT = config.redline?.dangerPercent ?? 25;
// ── Helper: status level ──────────────────────────────────────────────────────
...
async function analyseAxis(axis, thresholdPercent, from, to, last) {
    // Step 1: Historical peak for this axis column
    const peakSql = `SELECT MAX("${axis}") AS max_val, MIN("${axis}") AS min_val` +
        ` FROM public."${TORQUE_TABLE}"`;
    console.log('Redline peak SQL:', peakSql);
    const peakResult = await pool.query(peakSql);
    const maxVal = parseFloat(peakResult.rows[0]?.max_val ?? 0);
    const minVal = parseFloat(peakResult.rows[0]?.min_val ?? 0);
    ...
    const positiveLimit = maxVal * (thresholdPercent / 100);
    const negativeLimit = minVal * (thresholdPercent / 100);
    ...
    const windowSql = `SELECT "${axis}", timestamp FROM public."${TORQUE_TABLE}"` +
    ...
    const redlineReadings = readings.filter((r) => {
...
// ── Route handler ─────────────────────────────────────────────────────────────
redlineRouter.get('/hbot-torque/redline', async (req, res, next) => {
    ...
    const thresholdPercent = Math.min(Math.max(convertDataToInteger(req.query.threshold, DEFAULT_THRESHOLD_PERCENT), 1), 100);
    // Validate axis param
    if (axisParam !== '' && !TORQUE_AXES.includes(axisParam)) {
        res.status(400).json({
    ...
    try {
        const results = await Promise.all(axesToAnalyse.map(a => analyseAxis(a, thresholdPercent, from, to, last)));
        res.json({
