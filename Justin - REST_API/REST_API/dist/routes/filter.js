...
filterRouter.get('/:sensor/readings', async (req, res, next) => {
    ...
    const valMin = req.query['val-min'] !== undefined ? parseFloat(req.query['val-min']) : null;
    const valMax = req.query['val-max'] !== undefined ? parseFloat(req.query['val-max']) : null;
    const valEq = req.query['val-eq'] !== undefined ? parseFloat(req.query['val-eq']) : null;
    const valNeq = req.query['val-neq'] !== undefined ? parseFloat(req.query['val-neq']) : null;
    const valGt = req.query['val-gt'] !== undefined ? parseFloat(req.query['val-gt']) : null;
    const valLt = req.query['val-lt'] !== undefined ? parseFloat(req.query['val-lt']) : null;
    // ── Validate ──────────────────────────────────────────────────────────────
    ...
    for (const [name, v] of [['val-min', valMin], ['val-max', valMax], ['val-eq', valEq],
        ['val-neq', valNeq], ['val-gt', valGt], ['val-lt', valLt]]) {
        if (v !== null && isNaN(v)) {
    ...
    if (valCol) {
        if (valEq !== null) {
            values.push(valEq);
            conditions.push(`"${valCol}" = $${values.length}`);
        }
        if (valNeq !== null) {
            values.push(valNeq);
            conditions.push(`"${valCol}" != $${values.length}`);
